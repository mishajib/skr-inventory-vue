...
export function useProductsComposable() {
    // Reactive data
    const data = reactive({
        dataLoaded: false,
        dialog    : {
    ...
    const getItems = async () => {
        try {
            const response  = await axios.get(`/products?page=${data.pagination.current_page}`);
    ...
    const deleteItem = async () => {
        try {
            const response = await axios.delete(`/products/${data.dialog.id}`);
