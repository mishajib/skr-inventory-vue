...
export function usePurchasesComposable() {
    // Reactive data
    const data = reactive({
        dataLoaded: false,
        dialog    : {
    ...
    const getItems = async () => {
        try {
            const response  = await axios.get(`/purchases?page=${data.pagination.current_page}`);
            data.purchases  = response.data.data.data;
    ...
    const deleteItem = async () => {
        try {
            const response = await axios.delete(`/purchases/${data.dialog.id}`);
