...
export function useUnitsComposable() {
    // Reactive data
    const data = reactive({
        dataLoaded: false,
        dialog    : {
    ...
    const getItems = async () => {
        try {
            const response  = await axios.get(`/units?page=${data.pagination.current_page}`);
            data.units      = response.data.data.data;
    ...
    const deleteItem = async () => {
        try {
            const response = await axios.delete(`/units/${data.dialog.id}`);
